...
function rsvp_me_calendar_widget($options){
	...
	<div><? rsvp_me_draw_calendar(NULL) ?></div>
...
function rsvp_me_draw_calendar($obj, $month=NULL, $year=NULL, $settings=NULL){
	...
	*/
	...
	for($list_day = 1; $list_day <= $days_in_month; $list_day++):
		...
		$calendar.= '<td class="' . ($is_today ? 'calendar-today' : 'calendar-day') . '">';

			/** check for events !! **/
			
			if($events[$current_ymd]){
				/* add in the day number */
				$calendar .= '<div class="event-day" onclick="rsvpMe.showEvent(' . $events[$current_ymd]['id'] . ')">'.$list_day.'</div>';
				
				//let's also build append the hidden forms to the DOM
